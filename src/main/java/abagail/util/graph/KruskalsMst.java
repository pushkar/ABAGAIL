...
public class KruskalsMst implements GraphTransformation {
    ...
    public Graph transform(Graph g) {
        WeightedEdge[] edges = edgeArray(g);
        Arrays.sort(edges);
    ...
    private WeightedEdge[] edgeArray(Graph g) {
        // TODO: this is very awkward, needs to be revisited
        // ALSO, assuming that all edges are weighted edges is dubious
        Set<Edge> set = g.getEdges();
        WeightedEdge[] array = new WeightedEdge[set.size()];
        int i = 0;
        for (Edge e: set) {
            array[i++] = (WeightedEdge) e;
        }
        Arrays.sort(array);
        return array;
    }
